...
code more readable and maintainable. These helpers should be made as easy to
inline as possible by the JVM whenever possible.
...
- Add `mavenLocal()` as a repository. Gradle checks repositories in the order they are declared,
  so if you also have the Paper repository added, put this above that one.
